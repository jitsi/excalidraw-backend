...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", async (roomID) => {
    serverDebug(
      `${socket.id} has joined ${roomID} for url ${socket.conn.request.url}`,
    );
    ...
    io.in(roomID).emit(
      "room-user-change",
      sockets.map((clientSocket) => clientSocket.id),
    );
  });
    ...
    for (const roomID of Array.from(socket.rooms)) {
      ...
      if (!isFollowRoom && otherClients.length > 0) {
        socket.broadcast.to(roomID).emit(
          "room-user-change",
          otherClients.map((clientSocket) => clientSocket.id),
        );
      }
  ...
  socket.on("user-follow", async (payload: OnUserFollowedPayload) => {
    ...
    switch (payload.action) {
      case "FOLLOW": {
        ...
        const sockets = await io.in(roomID).fetchSockets();
        const followedBy = sockets.map((clientSocket) => clientSocket.id);
